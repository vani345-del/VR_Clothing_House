...
const AddProductPage = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 md:p-6 shadow rounded-md">
      <h2 className="text-2xl md:text-3xl font-bold mb-6 text-center">Add New Product</h2>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Input label="Sizes (comma separated)" name="sizes" value={productData.sizes.join(',')}
            onChange={(e) => setProductData({ ...productData, sizes: e.target.value.split(',').map(s => s.trim()) })} />
          <Input label="Colors (comma separated)" name="colors" value={productData.colors.join(',')}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-4">
            {productData.images.map((img, idx) => (
              <div key={idx} className="border rounded-md p-2">
                <img src={img.url} alt={img.altText || 'Product Image'} className="w-full h-32 object-cover mb-2 rounded-md" />
                ...
                <input type="text" placeholder="Color" value={img.color}
        ...
        <div className="flex flex-wrap gap-6 mt-4">
          <label><input type="checkbox" name="isFeatured" checked={productData.isFeatured} onChange={handleChange} /> Featured</label>
          <label><input type="checkbox" name="isPublished" checked={productData.isPublished} onChange={handleChange} /> Published</label>
        </div>

        <button type="submit" className="w-full bg-blue-600 text-white py-2 mt-6 rounded hover:bg-blue-700">
          Add Product
        </button>
      </form>
    </div>
